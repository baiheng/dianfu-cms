...
const NewForm = Form.create()(
    (props) => {
        const { visible, onCancel, onOk, form, title, confirmLoading, sender} = props;
                        ...
                        {getFieldDecorator('type', {
                            rules: [
                                ...
                                <Select  placeholder="请选择"
                                    getPopupContainer={() => document.getElementById('new-form-area')}>
                                {
                                    user.conf["notification.notification.type"].map((item, index) => {
...
const EditForm = Form.create()(
    (props) => {
        const { visible, onCancel, onOk, form, title, confirmLoading, data, sender} = props;
        const { getFieldDecorator } = form;
        return (
            <Modal
                visible={visible}
...
class Notification extends React.Component {
    ...
    getSenderList(){
        $.ajax({
            url: "/api/v1/notification/sender",
            type: "GET",
            dataType: "json",
            success: function(data){
                if(data.ret == 0){
                    this.setState({
                        sender: data.data.list,
                    });
                }else{
                    user.showRequestError(data)
                }
                        ...
                        <div className="am-g am-g-collapse">
                            ...
                            <div className="am-u-sm-3"> 
                                <div className="am-input-group am-input-group-default">
                                    <input type="text" className="am-form-field" placeholder="通知标题" ref="name" />
